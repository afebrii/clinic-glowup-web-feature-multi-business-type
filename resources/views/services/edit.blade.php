...
@section('page-title', __('service.edit'))
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 max-sm:p-4">
        <form action="{{ route('services.update', $service) }}" method="POST" enctype="multipart/form-data" class="space-y-6 max-sm:space-y-4">
            @csrf
            @method('PUT')
            ...
            @if($service->image)
                <div>
                    <label class="block text-sm max-sm:text-xs font-medium text-gray-700 dark:text-gray-300 mb-2 max-sm:mb-1.5">{{ __('service.current_image') }}</label>
                    <div class="flex items-center gap-4">
                        <img src="{{ Storage::url($service->image) }}" alt="{{ $service->name }}" class="w-24 h-24 rounded-lg object-cover">
                    </div>
                </div>
            @endif
            ...
            <!-- Image -->
            <div>
                <label for="image" class="block text-sm max-sm:text-xs font-medium text-gray-700 dark:text-gray-300 mb-2 max-sm:mb-1.5">{{ $service->image ? __('service.change_image') : __('common.image') }}</label>
                <input
                    type="file"
                    id="image"
                    name="image"
